...
void HelloTriangleApp::onWindowResized(GLFWwindow *window, int width,
                                       int height) {
  if (width == 0 || height == 0)
    return;
...
VkShaderModule
HelloTriangleApp::createShaderModule(const std::vector<char> &code) {
...
void HelloTriangleApp::createLogicalDevice() {
  ...
  if (enableValidationLayers) {
    cout << "Enabled layers " << validationLayers.size() << endl;
...
QueueFamilyIndices
HelloTriangleApp::findQueueFamilies(VkPhysicalDevice device) const {
...
HelloTriangleApp::querySwapChainSupport(VkPhysicalDevice device) const {
...
VKAPI_ATTR VkBool32 VKAPI_CALL HelloTriangleApp::dbgFunc(
...
BufferMemory HelloTriangleApp::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties,
  ...
  // tODO: use a Memory pool instead, multiple buffers in the same block
...
void HelloTriangleApp::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
  VkCommandBuffer commandBuffer = beginSingleTimeCommands();  // TODO: good place for a RAII object?
...
void HelloTriangleApp::createDescriptorSetLayout() {
  ...
  uboLayoutBinding.descriptorCount = 1; //could be multiple for an array of bones
...
// TODO: Tie with createDescriptorSetLayout
...
void HelloTriangleApp::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout,
  ...
  barrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED; // TOOD: to swap between queues
  ...
  if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
    ...
    throw std::invalid_argument("unsupFported layout transition!");
  }
